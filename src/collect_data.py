...
live countdown, then hold the gesture still.  Press Q to quit early.
Samples are saved (or appended) to data/<gesture>.npy.

For best cross-person generalisation, collect samples at varied distances
from the camera and with slightly different hand tilts.
"""
...
def collect(gesture: str, n_samples: int, output_dir: str, model_dir: str) -> None:
    # ── registry check ─────────────────────────────────────────────────────────
    if gesture not in GESTURES:
        known = ", ".join(sorted(GESTURES))
        print(f"Warning: '{gesture}' is not in the gesture registry.")
        print(f"  Known gestures: {known}")
        print(f"  Add it to src/config.py to silence this warning.\n")
    description = GESTURES.get(gesture, "")
    ...
    countdown_end: float | None = None   # set to time.time()+3 when SPACE pressed
    ...
    while cap.isOpened():
        ...
        # Previously cv2.waitKey(1000) was called in a tight loop, which froze
        # the camera feed during the countdown.  We now use wall-clock time so
        # the live preview keeps running and hand-skeleton stays visible.
        if countdown_end is not None and not recording:
            remaining = countdown_end - time.time()
            if remaining <= 0:
                recording     = True
                countdown_end = None
            else:
                cv2.putText(frame, f"Starting in {int(remaining) + 1}…", (160, 240),
        ...
        # Pose description — helps keep samples consistent across sessions
        if description:
            cv2.putText(frame, description, (10, 58),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1)

        if not recording and countdown_end is None:
            cv2.putText(frame, "SPACE = start recording   Q = quit", (10, 78),
        ...
        if key == ord(" ") and not recording and countdown_end is None:
            countdown_end = time.time() + 3   # begin 3-second live countdown
